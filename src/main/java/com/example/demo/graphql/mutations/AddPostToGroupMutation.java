...
@Component
@Slf4j
public class AddPostToGroupMutation implements DataFetcher<AddNewPostResponseDto>, ApplicationContextAware {
    ...
    public AddNewPostResponseDto get(DataFetchingEnvironment env) throws Exception {
        ...
        if (!StringUtils.hasText(username) || !StringUtils.hasText(groupId) || !StringUtils.hasText(text)) {
            throw new InvalidInput("Input validation error!", HttpStatus.BAD_REQUEST);
        }
        PostService postService = context.getBean(PostService.class);

        try {
            return postService.addPostToGroup(groupId, username, text);
        } catch (Exception e) {
